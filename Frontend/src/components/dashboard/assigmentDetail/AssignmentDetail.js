...
export default function AssignmentDetail({ assignmentDetail }) {
  ...
  var thisSubmittedAssignment =  findSubmittedAssignmentByStudentId(assignmentDetail._id, loginnedUserId._id) || null;
        ...
        <div className="AssignmentDetail-commit-comment">
          <div className="AssignmentDetail-commit">
            ...
            >
              ...
              {
                thisSubmittedAssignment.submissionFiles &&  <div>Đã Nộp</div>
              }
              {
                !thisSubmittedAssignment.submissionFiles && <div>Chưa Nộp</div>
              }
            ...
            <div>
              ...
              {thisSubmittedAssignment && (
                <div className="file-content">
                  {thisSubmittedAssignment.submissionFiles?.map((fileID, index) => {
                    const fileInfo = getFileInfo(fileID);
                    if (fileInfo) {
                      const { filename, path } = fileInfo;
                      const isPdf = filename.toLowerCase().endsWith(".pdf");
